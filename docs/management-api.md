...
- `400`：`proxyUrl`、`customHeaders`、`globalWeight` 等字段格式不合法
...
| `platformUserId` | `number` | 否 | 历史兼容站点可能需要手动传 |
...
> [!TIP]
> 如果历史站点返回 `needsUserId: true`，说明脚本里要补上 `platformUserId` 再重试。新建站点请优先使用 OpenAI、Claude 或 Gemini 适配。
...
| `platformUserId` | `number` | 否 | 历史兼容字段；新建 OpenAI、Claude、Gemini 站点通常不需要 |
...
```json
{
  "error": "An openai site with URL https://api.example.com already exists."
}
```
