...
<p>
Connect your OpenAI, Claude, and Gemini upstreams
<br>
...
Metapi unifies these upstream connections into **one endpoint (with configurable per-project downstream API Keys)**, so downstream tools can use one API key with automatic model discovery and smart routing.
...
&emsp;• Balance Management · Alerts & Notifications · Data Dashboard
&emsp;↓
**Upstream Platforms** (OpenAI · Claude · Gemini)
...
| **Claude** | `claude` | Claude / Anthropic APIs |
| **Gemini** | `gemini` | Gemini / Google AI APIs |

The three adapters cover model discovery, proxy integration, and routing. Account, balance, and token capabilities depend on the upstream API surface.
...
Alert scenarios: low balance warning, site/account anomalies, proxy request failures, token expiry reminders. Alert cooldown mechanism (default: 300 seconds) prevents duplicate notifications.
...
| Claude | Claude / Anthropic APIs |
| Gemini | Gemini / Google AI APIs |
