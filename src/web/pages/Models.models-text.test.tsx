...
describe('Models models text', () => {
  ...
  it('renders readable Chinese labels and fallback descriptions for models models', async () => {
    ...
    try {
      ...
      expect(initialText).toContain('模型');
      ...
      expect(expandedText).toContain('暂无描述');
      expect(expandedText).toContain('基础信息');
      expect(expandedText).toContain('站点');
      expect(expandedText).not.toContain('余额');
    } finally {
      root?.unmount();
    }
  });

  it('shows newly recognized brands in the models filter panel', async () => {
    apiMock.getModels.mockResolvedValue({
      models: [
        {
          name: 'nvidia/vila',
  ...
  it('shows user-reported recognizable brands in the models filter panel', async () => {
    apiMock.getModels.mockResolvedValue({
      models: [
        {
  ...
  it('keeps the mobile filter entry visible even while the first screen is still loading', async () => {
    ...
    apiMock.getModels.mockImplementation(() => new Promise(() => {}));
  ...
  it('limits expanded account detail to the selected site filter', async () => {
    apiMock.getModels.mockResolvedValue({
      models: [
        {
          name: 'gpt-4o',
