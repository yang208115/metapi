...
export default function ModelTester() {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      ...
      try {
        const [modelsResult, routesResult] = await Promise.allSettled([
          api.getModels(),
          api.getRoutes(),
        ]);

        if (modelsResult.status === 'rejected' && routesResult.status === 'rejected') {
          throw modelsResult.reason || routesResult.reason || new Error('failed to fetch models');
        }
